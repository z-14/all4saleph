...
$timestamp = mktime(date("H"),date("i"),date("s"), date("n") , date("j"), date("Y"));
		...
		if ($conn->query($sql) === TRUE) {
			include("notif.php");
				...
				{
  notif($u_id,$row["reciever_id"], $u_u.' Sent a messages', 'conversation.php?product_id='.$product_id.'&merchant_id='.$merchant_id.'&seller_id='.$seller_id.'&dd=no');
				}
				else
				{
 notif($row["reciever_id"],$row["sender_id"], $u_u.' Sent a messages', 'conversation.php?product_id='.$product_id.'&merchant_id='.$merchant_id.'&seller_id='.$seller_id.'&dd=no');
				}
